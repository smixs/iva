...
В 10:00 по локальному времени Iva без обращения к модели проверяет Git upstream. Пока новой стабильной версии `MAJOR.MINOR.PATCH` нет, сообщений не будет; каждую новую версию Iva предлагает только один раз. Если `TELEGRAM_DIGEST_CHAT_ID` пуст, используется первый доверенный ID.
