...
- **[28.07.2026] v0.3.4** 🧠 Уровень размышлений теперь зависит от модели: привычные кнопки Telegram показывают доступные варианты, а `low` / `medium` / `high` реально доходят до Ollama Cloud и OpenCode Go, как и до подписки OpenAI
- **[28.07.2026] v0.3.4** ♻️ `/new` теперь сбрасывает только текущий личный чат, диалог в группе или тему форума; `/restart` делает такой же точечный сброс и перезапускает процесс. Eve обновлён до 0.27.8, а `iva reset` остаётся явной аварийной командой для сброса всего агента
- **[28.07.2026] v0.3.4** 🛡️ При обновлении карточки памяти безопасно объединяются: неизвестные поля и старый текст больше не затираются. API Eve доступен только локально и требует bearer-ключ, секретный файл больше не проваливается в модель, а встроенный rich-post не может выгрузить заметки vault, логи или `.env`
