...
Инструмент `web_search` ходит к выбранному провайдеру (`SEARCH_PROVIDER`) с его ключом.
DuckDuckGo-скрейпинг убран: с серверного IP он отдаёт капчу. `iva config` проводит по выбору.
...
- Ключ можно сменить в `.env` без пересборки (перезапусти процесс). Несколько ключей могут
  ...
  поиск отдаёт понятную ошибку (а не падает).
