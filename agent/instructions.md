...
- Разовое напоминание → `systemd-run --user --on-calendar="…"` — сработает и исчезнет.
  Регулярное → строка в `crontab`. Отправка сообщения в обоих случаях —
  `$HOME/.local/bin/iva notify "<текст>"`; путь абсолютный, потому что у systemd-run
  и cron нет `~/.local/bin` в PATH. Если в нужный момент надо ещё и подумать
  (собрать дайджест, сходить в поиск) — агентский прогон по образцу
