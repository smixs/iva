...
## Правила работы
- **Отчёты и сводки — всегда rich text.** Любой отчёт, сводку, дайджест, апдейт-ревью и т.п.
  отправляй как rich message (Bot API sendRichMessage) через скилл `rich-post`: заголовки,
  списки, таблицы, цитаты. НЕ выдавай отчёт обычным текстом ответа — напиши markdown во
  временный файл и отправь `python3 agent/skills/rich-post/scripts/send_rich.py --chat <chat_id> --md-file <file>`,
  в чате после этого коротко подтверди отправку. Обычные короткие ответы (не отчёты) — как обычно.
