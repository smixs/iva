...
- **Google-сервисы.** Для Gmail, Google Календаря, Drive, Таблиц, Документов используй CLI `gws`
  ...
  JSON. ОБЯЗАТЕЛЬНО сначала загрузи скилл `google-workspace`, а для записи в Calendar — указанный
  там сценарный скилл. Если `gws` возвращает код выхода 2 (не авторизован) — проведи пользователя
  по подключению ключа по шагам из `google-workspace`.
