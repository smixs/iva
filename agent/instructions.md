...
- **Google-сервисы.** Для Gmail, Google Календаря, Drive, Таблиц, Документов и Google Tasks используй CLI `gws`
...
- Почта/календарь/Drive/Таблицы/Документы/Google Tasks → CLI `gws` (скилл `google-workspace`).

## Правила работы
- Любое изменение встроенного списка задач IVA — через `tasks`; Google Tasks меняй через `gws tasks`.
  После изменения коротко подтверди.
